...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        eval_sampler = SequentialSampler(eval_dataset) if args.local_rank <= 0 else DistributedSampler(eval_dataset)
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False, process_labels=None, get_raw_examples=False):
    ...
    if args.local_rank == 0 and not evaluate and torch.distributed.is_initialized():
        torch.distributed.barrier()  # Make sure only the first process in distributed training process the dataset, and the others will use the cache
