...
class BiRNNModel(nn.Module):
    ...
    def forward(self, sent_tuple):
        ...
        sent = fetch_embedding(self, self.mode, sent, squash=True)
        ...
        if hasattr(self, "output_hidden_states") and self.output_hidden_states:
            if not hasattr(self, "padding_idx"):
                raise ValueError("The padding_idx is not set but is needed for unpacking packed sequences!")
            return torch.nn.utils.rnn.pad_packed_sequence(rnn_seq, batch_first=True, padding_value=self.padding_idx)[0]
        else:
