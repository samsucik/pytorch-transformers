...
def init_embedding(model, args):
    rand_embed_init = torch.Tensor(args.vocab_size, args.token_embedding_dimensionality)\
        .uniform_(-0.25, 0.25)
    if args.mode == "rand":
        model.embed = nn.Embedding.from_pretrained(rand_embed_init, freeze=False)
        model.embedding_dim = args.token_embedding_dimensionality
    elif args.mode == "static":
        model.static_embed = nn.Embedding.from_pretrained(rand_embed_init, freeze=True)
        model.embedding_dim = args.token_embedding_dimensionality
    elif args.mode == "non-static":
        model.non_static_embed = nn.Embedding.from_pretrained(rand_embed_init, freeze=False)
        model.embedding_dim = args.token_embedding_dimensionality
    elif args.mode == "multichannel":
        model.static_embed = nn.Embedding.from_pretrained(rand_embed_init, freeze=True)
        model.non_static_embed = nn.Embedding.from_pretrained(rand_embed_init, freeze=False)
        model.embedding_dim = args.token_embedding_dimensionality * 2
    else:
